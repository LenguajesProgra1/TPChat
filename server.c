...
void dostuff(int); void error(const char *msg)
...
error("ERROR de apertura de socket"); bzero((char *) &serv_addr, sizeof(serv_addr)); portno = atoi(argv[1]);

serv_addr.sin_family = AF_INET; serv_addr.sin_addr.s_addr = INADDR_ANY; serv_addr.sin_port = htons(portno);

if (bind(sockfd, (struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0) error("ERROR en la conexión");
...
if (newsockfd < 0) error("ERROR en aceptar"); pid = fork();

if (pid < 0) error("ERROR en fork");

if (pid == 0)

{ close(sockfd); dostuff(newsockfd); exit(0); } else close(newsockfd);
...
printf("Aquí está el mensaje: %s\n",buffer); n = write(sock,"Tengo el mensaje",18); if (n < 0)
